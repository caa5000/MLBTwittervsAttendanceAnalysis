...
#ratio of twitter followers to tv market size viewing area? Which teams have the highest?
  ...
  #Determine if there is a relationship between follower/market ratio to avg % of stadium filled.
    ...
    legend("bottomright", legend = "Data shows a relationship (p-value .00194)", col = "blue", lwd=1)
      ...
      ggplot(teamsData, aes(x = teamsData$FollowerToTVMarketRatio, y = PercStadiumFill2015, colour = X2015WP)) + geom_point(aes(size = X2015WP)) + scale_colour_gradient("2015 Win %",low = "red", high = "green") + theme(legend.title=element_text(colour="red", size = 14, face="bold"),legend.position="right") + labs(x = "Ratio of Twitter Followers to size of TV Market (markets with 2 teams halved)", y = "% of stadium fill for 2015 Home Games", title = "Does ratio of twitter followers in a TV Market correlate with MLB attendance?", size="2015 Win %", colour="2015 Win %") + geom_abline(intercept=0.5023, slope=1.4187, color="blue", size=.5, linetype=2) + annotation_custom(myText) + annotation_custom(slopeData)
      #Plot with the team names for further looks.
      ggplot(teamsData, aes(x = teamsData$FollowerToTVMarketRatio, y = PercStadiumFill2015, label = TeamName)) + geom_label(aes(fill = X2015WP), color="white", fontface = "bold", show.legend=TRUE) + scale_colour_manual(values=c("red", "blue")) + labs(x = "Ratio of Twitter Followers in each TV Market", y = "% of stadium fill for 2015 Home Games", title = "Does ratio of twitter followers in a TV Market correlate with attendance?", label="2015 Win %", colour="2015 Win %", fontface="bold")
      ...
      ggplot(teamsData, aes(x = teamsData$FollowerToTVMarketRatio, y = PercStadiumFill2015, size=3)) + geom_point(aes(color = factor(teamsData$PayrollRanking))) + scale_color_manual(values = c("red", "yellow", "green")) + theme(legend.title=element_text(colour="blue", size = 14, face="bold"),legend.position="right") + labs(x = "Ratio of Twitter Followers to size of TV Market (markets with 2 teams halved)", y = "% of stadium fill for 2015 Home Games", title = "How is payroll, twitter ratio and stadium fill associated?", colour="2015 Payroll Ranking", size="") + geom_abline(intercept=0.5023, slope=1.4187, color="blue", size=.5, linetype=2) + annotation_custom(payrollInfo) + annotation_custom(slopeData)
